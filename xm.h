...
void
save_song_to_file (const char *output_filename, XM_song *song)
{
  /* NOTE(accacio): maybe it would be better to save to a buffer and then use
   * only one fwrite */
  ...
  for (int pat_idx = 0; pat_idx < song->header.n_patterns; pat_idx++)
    {
      ...
      uint32_t max_pattern_size
          = 5 * pattern->n_channels * pattern->header.n_rows;
      uint8_t *compact_pattern = (uint8_t *) malloc (max_pattern_size);
